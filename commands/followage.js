exports.run = async (client, chat, channel, commandData) => {
    ...
    const follow = await greddBot.Utils.APITwitch.getSubage(username, Rchannel)

    if (follow["followedAt"]) {
        const ms = new Date().getTime() - Date.parse(follow["followedAt"])
        chat.send(channel, `@${commandData.user.name}, Пользователь ${username} подписан на ${Rchannel} ${greddBot.Utils.misc.humanizeDuration(ms)}.`)
    } else {
