...
exports.run = async (client, chat, channel, commandData) => {
    ...
    try {
        if (msgArgs && (isMod || isBroad) && greddBot.Config.owner ) {
            await greddBot.Utils.ApiClient.helix(`games?name=${msgArgs}`)
            .then(async (response) => {
                if (response.data.length > 0) {
                    const gameId = response.data[0].id;
                    await greddBot.Utils.ApiClient.helix(`channels?broadcaster_id=176257472`, {
                        method: "PATCH",
                        data: {
                            game_id: gameId
                        },
        ...
        console.log(error)
        greddBot.Logger.error(`${pc.red(`[ERROR]`)} || Error: ${error}`)
    }
}
